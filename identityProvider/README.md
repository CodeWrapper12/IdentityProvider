...
To update the database use this command from the Web project folder (replace `identityProvider` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../identityProvider.Infrastructure/identityProvider.Infrastructure.csproj -s identityProvider.Web.csproj
```
